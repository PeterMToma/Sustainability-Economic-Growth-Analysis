# Sustainability-Economic-Growth-Analysis
This project analyzes the relationship between sustainability and economic growth across selected countries from 2010 t0 2024. It develops a composite Sustainability Index (CSI) based on environmental, economic, social, energy transition, and smart sustainability dimensions to compare countries and assess sustainable development progress over time.

# Project Title
Measuring the Impact of Sustainability on Economic Growth Across Countries

# Countries to be compared
  # Europe
- United Kingdom
- Germany
- Sweden
- France
  
  # Asia
- Japan
- China
- Indonesia
  
  # Middle East
- Saudi Arabia
- United Arab Emirates
- Egypt
  
  # Americas
- United States
- Brazil
- Canada
  
  # Africa
- South Africa

 # Project Period
  2010-2024

 # Taregt Question  
  1- What is the impact of sustainability on economic growth across the selected countries?

# Sub-Questions
  1- What is the sustainability level in each country from 2010 to 2024?
  
  2- How does sustainability differ between developed and developing countries?
  
  3- Which sustainability dimension has the strongest impact on economic growth?
  
  4- Is renewable energy adoption associated with higher economic performance?
  
  5- Does environmental performance significantly influence long-term economic growth?
  
  6- How have countries improved in sustainability over time?
  
  7- Which region shows the fastest progress in sustainability indicators?
  
  8- Is there a measurable correlation between the Composite Sustainability Index (CSI) and GDP growth?
  
  9- Which countries are the best and worst performers in sustainability?

# Final Sustainability Framework
  # Environmental Index
- CO₂ Emissions
- PM2.5 Air Pollution
- Renewable Energy Consumption
- Energy Intensity
- Waste Management / Recycling Rate
  
  # Economic Index
- GDP Growth Rate
- GDP per Capita
- Employment Rate
- Industrial Value Added
- Inflation Rate
  
  # Social Index
- Education Index
- Life Expectancy
- Poverty Rate
- Access to Clean Water
  
  # Energy & Transition Index
- Renewable Energy Share
- Fossil Fuel Consumption
- Electricity Access
- Energy Consumption per Capita
  
  # Smart Sustainability Index
- Smart Cities Development Index
- Rural Digitalization Index
- Digital Infrastructure for Sustainability
- Recycling Rate

# Methodology
1- Data Collection (2010–2024)
2- Data Cleaning & Preprocessing
3- Missing Value Handling
4- Normalization
5- Negative Indicator Reversal
6- Sub-Index Construction
7- Composite Sustainability Index (CSI)
8- Correlation Analysis
9- Regression Analysis
10- Country Ranking
11- Trend Analysis

# Expected Outcomes
- Composite Sustainability Index (CSI)
- Country Sustainability Ranking
- Trend Analysis (2010–2024)
- Correlation between CSI and GDP Growth
- Regression-based impact of sustainability on economic growth
- Regional comparison (Developed vs Developing)

كده الم
